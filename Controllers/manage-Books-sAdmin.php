...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['title'], $_POST['author'], $_POST['category'], $_POST['copies'])) {
    ...
    if (!empty($title) && !empty($author) && !empty($category) && $copies > 0) {
        ...
        if ($stmt->execute()) {
            ...
            redirect('Views/dash/admin/super-Admin-Dashboard.php'); // Same folder redirect
        } else {
            $_SESSION['message'] = 'Error adding book: ' . $conn->error;
            redirect('Views/dash/admin/super-Admin-Dashboard.php');
        }
        $stmt->close();
    } else {
...
if (isset($_GET['delete_id'])) {
    ...
    $check_stmt->close();
    redirect('Views/dash/admin/super-Admin-Dashboard.php'); // Always redirect to the same folder
}
