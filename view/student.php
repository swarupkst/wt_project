...
// Total books
$sqlTotalBooks = "SELECT COUNT(*) AS total FROM books";
$totalBooks = $conn->query($sqlTotalBooks)->fetch_assoc()['total'];

// Borrowed count
$sqlBorrowCount = "SELECT COUNT(*) AS borrowed FROM borrowed_books WHERE student_id = $student_id AND return_date IS NULL";
$borrowedCount = $conn->query($sqlBorrowCount)->fetch_assoc()['borrowed'];
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
function searchBooks() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4 && xhr.status === 200) {
            document.querySelector(".book-container").innerHTML = xhr.responseText;
        }
    };
    xhr.send();
}
...
<body>
    ...
    <div class="stats-container">
        <div class="stat-card total">📚 Total Books: <b><?php echo $totalBooks; ?></b></div>
        <div class="stat-card borrowed">📖 My Borrowed: <b><?php echo $borrowedCount; ?></b></div>
    </div>

    <!-- Books & Borrowed Side by Side -->
        ...
        <div class="table-box">
            ...
            <table>
                ...
                <tbody>
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            $rowClass = (is_null($row['return_date']) && strtotime($row['due_date']) < time()) ? "class='overdue'" : "";
                            echo "<tr $rowClass>
                                    <td>{$row['title']}</td>
